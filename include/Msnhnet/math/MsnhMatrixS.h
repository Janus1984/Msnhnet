...
public:
    ...
    MatS(const MatS& mat) : mWidth(mat.mWidth), mHeight(mat.mHeight) {
        int mN = dataNum();
    ...
    static MatS dense(const int w, const int h, const T& t) {
    ...
    static MatS diag(const int num, const T& t) {
        assert(num <= maxH && num <= maxW);
    ...
    static MatS random(const int w, const int h) {
        assert(h <= maxH && w <= maxW);
    ...
    static MatS randomDiag(const int num) {
        assert(num <= maxH && num <= maxW);
    ...
    MatS transpose() const {
        MatS tmpMat(mHeight, mWidth);

        if (mHeight == mWidth) {
            for (int i = 0; i < this->mHeight; ++i) {
                for (int j = i; j < this->mWidth; ++j) {
                    tmpMat.mValue[(j * mHeight) + i] =
                        mValue[(i * mWidth) + j];
                    tmpMat.mValue[(i * mHeight) + j] =
                        mValue[(j * mWidth) + i];
                }
            }
        } else {
    ...
    T det() const {
        ...
        if (this->mHeight == 2) {
            ...
            val = mValue[0] * mValue[4] * mValue[8] +
                  mValue[1] * mValue[5] * mValue[6] +
                  mValue[2] * mValue[3] * mValue[7] -
                  mValue[0] * mValue[7] * mValue[5] -
                  mValue[3] * mValue[1] * mValue[8] -
                  mValue[2] * mValue[4] * mValue[6];
        } else {
            MatS tmpMat = *this;
            ...
            for (int i = 0; i < m; i++) {
                ...
                for (int j = i + 1; j < m; j++) {
                    if (std::abs(tmpMat.mValue[(j * m) + i]) >
                        std::abs(tmpMat.mValue[(k * m) + i])) {
                        k = j;
                    }
                }

                if (std::abs(tmpMat.mValue[(k * m) + i]) <
    ...
    std::vector<MatS> LUDecomp(bool outLU) const {
        assert(mWidth == mHeight);
        MatS A = *this;
        int m = A.mWidth;
        int k = 0;
        ...
        for (int i = 0; i < m; i++) {
            ...
            for (int j = i + 1; j < m; j++) {
                if (std::abs(A.mValue[(j * m) + i]) >
                    std::abs(A.mValue[(k * m) + i])) {
                    k = j;
                }
            }

            if (std::abs(A.mValue[(k * m) + i]) <
                std::numeric_limits<T>::epsilon()) {
            ...
            for (int j = i + 1; j < m; j++) {
                ...
                if (!outLU) {
                    for (k = i + 1; k < m; k++) {
                        A.mValue[(j * m) + k] +=
                            alpha * A.mValue[(i * m) + k];
                    }

                    for (k = 0; k < m; k++) {
                        B.mValue[(j * m) + k] +=
                            alpha * B.mValue[(i * m) + k];
                    }
                } else {
                    for (k = i; k < m; k++) {
                        if (k < j) {
    ...
    std::vector<MatS> choleskyDeComp(bool outChols = true) const {
        assert(mWidth == mHeight);
        ...
        for (int i = 0; i < m; i++) {
            ...
            if (s < std::numeric_limits<T>::epsilon()) {
                throw Exception(1,
                    "[Mat]: Not a good matrix for cholesky! \n", __FILE__,
        ...
        if (!outChols) {
            for (int i = 0; i < m; i++) {
                for (int j = 0; j < m; j++) {
                    ...
                    for (int k = 0; k < i; k++) {
                        s -=
                            L.mValue[(i * m) + k] * eye.mValue[(k * m) + j];
                    }
            ...
            for (int i = m - 1; i >= 0; i--) {
                for (int j = 0; j < m; j++) {
                    ...
                    for (int k = m - 1; k > i; k--) {
                        s -=
                            L.mValue[(k * m) + i] * eye.mValue[(k * m) + j];
                    }
                    eye.mValue[(i * m) + j] = s * L.mValue[(i * m) + i];
                }
            }
    ...
    std::vector<MatS> eigen(const bool sort = true,
        const bool forceCheckSymmetric = false) const {
        assert(mWidth == mHeight);

        if (forceCheckSymmetric) {
            for (int i = 0; i < mHeight; ++i) {
                for (int j = i + 1; j < mWidth; ++j) {
                    if (mValue[(i * mWidth) + j] !=
                        mValue[(j * mWidth) + i]) {
                        throw Exception(1,
                            "[MatS]: not a symmetric matrix! \n", __FILE__,
                            __LINE__, __FUNCTION__);
                    }
                }
            }
        ...
        for (int k = 0; k < n; ++k) {
            ...
            if (k < n - 1) {
                for (maxIdx = k + 1, maxVal = std::abs(A[(n * k) + maxIdx]),
                    i = k + 2;
                    i < n; i++) {
            ...
            if (k > 0) {
                for (maxIdx = 0, maxVal = std::abs(A[k]), i = 1; i < k;
                    i++) {
        ...
        if (n > 1) {
            for (int iters = 0; iters < maxIters; iters++) {
                ...
                for (k = 0, maxVal = std::abs(A[indR[0]]), i = 1; i < n - 1;
                    i++) {
...
#undef rotate
                ...
                for (int j = 0; j < 2; j++) {
                    int idx = j == 0 ? k : l;
                    if (idx < n - 1) {
                        for (m = idx + 1, maxVal = std::abs(A[n * idx + m]),
                            i = idx + 2;
                            i < n; i++) {
                    ...
                    if (idx > 0) {
                        for (m = 0, maxVal = std::abs(A[idx]), i = 1;
                            i < idx; i++) {
        ...
        if (sort) {
            for (int k = 0; k < n - 1; k++) {
                int m = k;
                for (int i = k + 1; i < n; i++) {
                    if (std::abs(eigenvalues[m]) <
                        std::abs(eigenvalues[i])) {
                        m = i;
                    }
                }
    ...
    double SIGN(double a, double b) {
        return (
            ((b) >= 0.0 && fabs(b) > MSNH_F64_EPS) ? fabs(a) : -fabs(a));
    }

    int householderSVD(std::vector<VectorXSDS>& U, VectorXSDS& D,
        std::vector<VectorXSDS>& V, int maxiter) {
        ...
        bool maxarg1;
        bool maxarg2;
        double anorm(0), c(0), f(0), h(0), s(0), scale(0), x(0), y(0), z(0),
            g(0);
        ...
        for (i = 0; i < width; i++) {
            ...
            anorm =
                static_cast<double>(maxarg1 > maxarg2 ? maxarg1 : maxarg2);
        }
        ...
        for (i = width - 1 < height - 1 ? width - 1 : height - 1; i >= 0;
            i--) {
            ppi = i + 1;
            g = D(i);
            for (j = ppi; j < width; j++) {
        ...
        for (k = width - 1; k >= 0; k--) { /* Loop over singular values. */
            for (its = 1; its <= maxiter;
                its++) { /* Loop over allowed iterations. */
                flag = true;
    ...
    void jacobiSVD(MatS& At, MatS& _W, MatS& Vt) const {
        ...
        for (int i = 0; i < n - 1; i++) {
            ...
            if (i != j) {
                ...
                for (int k = 0; k < m; k++) {
                    std::swap(
                        At.mValue[(i * m) + k], At.mValue[(j * m) + k]);
                }

                for (int k = 0; k < n; k++) {
                    std::swap(
                        Vt.mValue[(i * n) + k], Vt.mValue[(j * n) + k]);
                }
            }
        }
    ...
    MatS getCol(const unsigned int col) const {
    ...
    void setCol(const unsigned int col, const MatS& mat) {
        assert(col < (unsigned int)mWidth);
    ...
    MatS getRow(const unsigned int row) {
    ...
    void setRow(const unsigned int row, const MatS& mat) {
        assert(row < (unsigned int)mHeight);
    ...
    VectorXSDS mulVec(const VectorXSDS& vec) const {
    ...
    void print() {
        if (isF32Mat()) {
            std::cout << "{MatFS  width: " << this->mWidth
                      << " , height: " << this->mHeight << '\n';
            std::cout << "    [" << '\n';
            for (int i = 0; i < this->mHeight; ++i) {
            ...
            std::cout << "{MatDS  width: " << this->mWidth
                      << " , height: " << this->mHeight << '\n';
            std::cout << "    [" << '\n';
            for (int i = 0; i < this->mHeight; ++i) {
    ...
    T value(const int w, const int h) const {
        assert(w < mWidth && h < mHeight);
        return mValue[(h * mWidth) + w];
    }

    T operator()(const int w, const int h) const {
        assert(w < mWidth && h < mHeight);
        return mValue[(h * mWidth) + w];
    }

    T& operator()(const int w, const int h) {
        assert(w < mWidth && h < mHeight);
        return mValue[(h * mWidth) + w];
    }
    ...
    static MatS eleWiseMul(const MatS& A, const MatS& B) {
    ...
    static MatS eleWiseDiv(const MatS& A, const MatS& B) {
        assert(A.mWidth == B.mWidth && A.mHeight == B.mHeight);
    ...
    friend MatS operator+(const MatS& A, const MatS& B) {
        assert(A.mWidth == B.mWidth && A.mHeight == B.mHeight);
    ...
    friend MatS operator+(const T& a, const MatS& A) {
        MatS mat(A.mWidth, A.mHeight);
    ...
    friend MatS operator+(const MatS& A, const T& a) {
        MatS mat(A.mWidth, A.mHeight);
    ...
    friend MatS operator-(const MatS& A) {
        MatS mat(A.mWidth, A.mHeight);
        ...
        if (B.mWidth == 1) {
            for (int i = 0; i < A.mHeight; ++i) {
                ...
                for (int j = 0; j < B.mHeight / 4; ++j) {
                    sum += A.mValue[(i * A.mWidth) + (j << 2) + 0] *
                           B.mValue[(j << 2) + 0];
                    sum += A.mValue[(i * A.mWidth) + (j << 2) + 1] *
                           B.mValue[(j << 2) + 1];
                    sum += A.mValue[(i * A.mWidth) + (j << 2) + 2] *
                           B.mValue[(j << 2) + 2];
                    sum += A.mValue[(i * A.mWidth) + (j << 2) + 3] *
                           B.mValue[(j << 2) + 3];
                }
            ...
            if (A.isF32Mat()) {
#ifdef USE_X86
                Gemm::cpuGemm(0, 0, A.mHeight, B.mWidth, A.mWidth, 1,
                    (float*)A.mValue, A.mWidth, (float*)B.mValue, B.mWidth,
                    1, (float*)mat.mValue, mat.mWidth,
                    SimdInfo::supportAVX2);
#else
                Gemm::cpuGemm(0, 0, A.mHeight, B.mWidth, A.mWidth, 1,
                    (float*)A.mValue, A.mWidth, (float*)B.mValue, B.mWidth,
                    1, (float*)mat.mValue, mat.mWidth, false);
#endif
            } else {
#ifdef USE_X86
                Gemm::cpuGemm(0, 0, A.mHeight, B.mWidth, A.mWidth, 1,
                    (double*)A.mValue, A.mWidth, (double*)B.mValue,
                    B.mWidth, 1, (double*)mat.mValue, mat.mWidth,
                    SimdInfo::supportAVX2);
#else
                Gemm::cpuGemm(0, 0, A.mHeight, B.mWidth, A.mWidth, 1,
                    (double*)A.mValue, A.mWidth, (double*)B.mValue,
                    B.mWidth, 1, (double*)mat.mValue, mat.mWidth, false);
#endif
            }
        }
    ...
    static T dotProduct(const MatS& A, const MatS& B) {
        assert(A.mWidth == B.mWidth && A.mHeight == B.mHeight);
    ...
    friend bool operator==(const MatS& A, const MatS& B) {
...
using MatSDS = MatS<8, 8, double>;
using MatMDS = MatS<16, 16, double>;
